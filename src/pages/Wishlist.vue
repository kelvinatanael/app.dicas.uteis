<template>
  <f7-page class="wishlist" name="wishlist">
    <f7-navbar>
      <a class="link" href="/">
        <svg
          aria-hidden="true"
    ...
    <f7-block class="content-wishlist">
      <div class="content-wishlist-itens">
        ...
        <div class="content-elements">
          <template v-if="wishlistUser.length > 0">
            <template v-for="(post, index) in wishlistUser">
              <template v-if="!searchWishlist || post.title.toLowerCase().indexOf(searchWishlist) !== -1">
                <card-tips :key="index" :tipsProps="post" :tipId="post.tipId"/>
              </template>
            </template>
          </template>
          <template v-else>
            <div class="content-not-wishlist">
              <h2>Você não adicionou nenhum favorito &#128543;</h2>
              <img class="img-fluid" src="static/icons/wish-list.png" alt="icon wishlist" title="icon wishlist"/>
            </div>
          </template>
        </div>
      </div>
...
export default {
  ...
  methods: {
    updateFilter(event) {
      this.searchWishlist = event.target.value.toLowerCase();
    },
